...
export default function LoginPage() {
  ...
  const redirectBasedOnRoleAndClaims = async (user: any) => {
    console.log("Redirecting user with email:", user.email);
    ...
    if (idTokenResult.claims.mustChangePassword) {
      ...
      console.log("Email matches admin email, redirecting to admin dashboard");
      setIsRedirecting(true);
      router.push("/dashboards/admin");
    } else {
      console.log("Regular user, redirecting to driver dashboard");
      setIsRedirecting(true);
              ...
              <div className="max-w-md mx-auto">
                ...
                <form onSubmit={handleSubmit} className="space-y-5">
                  ...
                  <div className="flex items-center justify-between">
                    <label className="flex items-center">
                      <input 
                        type="checkbox" 
                        className="w-4 h-4 bg-slate-800 border-gray-700 rounded text-yellow-400 focus:ring-yellow-500 focus:ring-offset-0" 
                      />
                ...
                <div className="mt-6 flex items-center justify-center gap-4 text-xs text-gray-600">
                  <span className="flex items-center gap-1">
                    <Shield className="w-3 h-3 text-yellow-400/50" /> Secure Login
                  </span>
                  <span className="w-1 h-1 bg-yellow-500/30 rounded-full"></span>
                  <span className="flex items-center gap-1">
                    <Clock className="w-3 h-3 text-yellow-400/50" /> 24/7 Access
                  </span>
                </div>
              </div>
            </div>
