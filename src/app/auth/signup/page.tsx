...
export default function RegisterPage() {
  ...
  // Get admin emails from environment variables (comma-separated list)
  useEffect(() => {
    const adminEmailsString = process.env.NEXT_PUBLIC_ADMIN_EMAILS || '';
    // Just use the first admin email for display purposes
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // signUp expects: email, password, displayName, role, phone
      await signUp(
        formData.email,
        formData.password,
        formData.fullName,
  ...
  const handleGoogleSignUp = async () => {
    ...
    try {
      // signInWithGoogle doesn't take any arguments in your implementation
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-slate-950 via-slate-900 to-slate-950 flex items-center justify-center p-4 font-sans overflow-x-hidden">
      {/* Taxi-themed animated background */}
      ...
      <div className="relative w-full max-w-5xl">
        <div className="bg-slate-900/80 backdrop-blur-xl rounded-3xl shadow-2xl border border-yellow-500/20 overflow-hidden">
          <div className="grid md:grid-cols-2 min-h-[650px]">
            {/* Left Column - Logo & Branding with Taxi Theme */}
            <div className="relative bg-gradient-to-br from-slate-900 to-slate-800 p-8 md:p-12 flex flex-col items-center justify-center overflow-hidden order-1 md:order-1 border-r border-yellow-500/20">
              {/* Decorative Elements - Taxi themed */}
              ...
              {/* Checkerboard pattern (taxi inspired) */}
              ...
              <div className="relative text-center md:text-left flex flex-col items-center md:items-start">
                ...
                {/* Company Name and Slogan with taxi colors */}
            ...
            <div className="p-8 md:p-10 order-2 md:order-2">
              <div className="max-w-md mx-auto">
                {/* Header with taxi styling */}
                <div className="mb-8">
                ...
                <form onSubmit={handleSubmit} className="space-y-5">
                  ...
                  {formData.password && (
                    <div className="space-y-2">
                      <div className="flex gap-1 h-1">
                        <div className={`flex-1 h-full rounded-full transition-colors ${
                          formData.password.length >= 6 ? 'bg-yellow-400' : 'bg-gray-700'
                        }`}></div>
                        <div className={`flex-1 h-full rounded-full transition-colors ${
                          /[A-Z]/.test(formData.password) ? 'bg-yellow-400' : 'bg-gray-700'
                        }`}></div>
                        <div className={`flex-1 h-full rounded-full transition-colors ${
                          /[0-9]/.test(formData.password) ? 'bg-yellow-400' : 'bg-gray-700'
                        }`}></div>
                        <div className={`flex-1 h-full rounded-full transition-colors ${
                          /[^A-Za-z0-9]/.test(formData.password) ? 'bg-yellow-400' : 'bg-gray-700'
                        }`}></div>
                      </div>
                  ...
                  <div className="flex items-start">
                    ...
                    <span className="ml-2 text-sm text-gray-400">
                      I agree to the{' '}
                      <Link href="/terms" className="text-yellow-400 hover:text-yellow-300 font-medium">
                        Terms of Service
                      </Link>{' '}
                      and{' '}
                      <Link href="/privacy" className="text-yellow-400 hover:text-yellow-300 font-medium">
                        Privacy Policy
                      </Link>
                    </span>
                  </div>
                ...
                <div className="mt-6 text-center">
                  <p className="text-gray-400">
                    ...
                    <Link href="/auth/login" className="text-yellow-400 hover:text-yellow-300 font-medium transition">
                      Sign In
                    </Link>
                  </p>
                </div>
                ...
                <div className="mt-8">
                  ...
                  <button
                    ...
                    className="w-full bg-slate-800 hover:bg-slate-700 text-white font-semibold py-3 rounded-xl transition-all shadow-md flex items-center justify-center gap-3 disabled:opacity-50 disabled:cursor-not-allowed border border-yellow-500/20 hover:border-yellow-400/50 hover:shadow-lg group"
                  >
                    {googleLoading ? (
                ...
                <div className="mt-6 flex items-center justify-center gap-4 text-xs text-gray-600">
                  <span className="flex items-center gap-1">
                    <Shield className="w-3 h-3 text-yellow-400/50" /> Secure
                  </span>
                  <span className="w-1 h-1 bg-yellow-500/30 rounded-full"></span>
                  <span className="flex items-center gap-1">
                    <Clock className="w-3 h-3 text-yellow-400/50" /> 24/7 Support
                  </span>
                </div>
              </div>
            </div>
        ...
        {/* Back to Home with taxi styling */}
